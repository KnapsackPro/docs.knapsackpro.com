...
To enable Retry only Failures, `npm install @knapsack-pro/playwright` to v0.3 or later.
...
- Node 0: executes `a.spec.ts` and `b.spec.ts` where `a.spec.ts:10` fails and `b.spec.ts:20` is skipped
- Node 1: executes `c.spec.ts` successfully

If you retry Node 0, Retry only Failures will execute only `a.spec.ts:10`. If you retry Node 1, Retry only Failures will execute no tests.
