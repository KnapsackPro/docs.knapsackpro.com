...
If you use spot/preemptible CI nodes like [Google Cloud Preemptible VMs](https://cloud.google.com/solutions/spot-vms) or [Amazon EC2 Spot Instances](https://aws.amazon.com/ec2/spot/), you also want a Fixed Split: when a preempted CI node restarts because of a manual or automatic retry, it re-runs the previous subset of tests before connecting to the queue to continue with the remaining ones.
...
- [Knapsack Pro Ruby - Troubleshooting](troubleshooting.mdx)
- [Using Knapsack Pro with RSpec](rspec.mdx)
- [Using Knapsack Pro with Cucumber](cucumber.mdx)
