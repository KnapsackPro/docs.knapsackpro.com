...
## Load code only once for a specific type of specs using `when_first_matching_example_defined`

You can save time and run tests faster if you avoid loading a service that is not needed to run tests.

For example, your system tests require `MyExpensiveService` to run. Tests are allocated dynamically between parallel CI nodes in Queue Mode. A CI node may not have a system test assigned to it. There is no point in booting `MyExpensiveService` in such a case.

You can configure RSpec to load the service dependency only when needed. It is loaded only once after the first system test is fetched from the Queue API.

Please add the following helper to your `spec_helper.rb`:
...
Update the `knapsack_pro` gem to `>= 7.0` in order to run them only once. Learn more about [hooks](hooks.mdx).
