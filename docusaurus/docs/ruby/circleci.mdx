...
KnapsackPro::Hooks::Queue.after_queue do |_queue_id|
  # `JUNIT_REPORT` must be the same path as `--out`
  # `JUNIT_REPORT` must be a full path (no `~`)
  JUNIT_REPORT = "tmp/rspec.xml"

  if File.exist?(JUNIT_REPORT) && ENV['CIRCLE_TEST_REPORTS']
    # highlight-start
    FileUtils.cp(JUNIT_REPORT, "#{ENV['CIRCLE_TEST_REPORTS']}/rspec.xml")
    # highlight-end
  end
end
```
