...
Knapsack Pro prints the Ruby threads' backtraces when killed with `TERM`:
...
You can use [the `timeout` program](https://man7.org/linux/man-pages/man1/timeout.1.html) to send the `TERM` signal if the CI provider does not kill the frozen process.

```bash
timeout --signal=TERM 600 bundle exec rake "knapsack_pro:queue:rspec[--format d]"
```

Notice that `600` is in seconds. It sends the signal after 10 minutes of running the command. You may want to adjust that number to ensure the `TERM` signal is sent after the process is stuck and not before.
