...
The main thread was killed due to the following `b_spec.rb`:

```ruby title="b_spec.rb"
...
The non-main thread was stuck at the time due to the following `sleep` in `a_spec.rb`:

```ruby title="a_spec.rb"
