...
Assuming you use 2 threads per Puma process, then it's good to have `pool` option set to 2 + some spare connections to avoid a problem when one of the DB connections is dead. When a dead connection happens then, ActiveRecord can use spare connections (not opened connections yet) to open a new connection.
