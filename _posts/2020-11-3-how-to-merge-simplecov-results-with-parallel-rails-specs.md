---
layout: post
title:  "How to merge SimpleCov results with parallel Rails specs"
...
So now when SimpleCov a creates a `.resutset.json` it will have a specific key depending on which CI node it was run like the example below. This will be useful down the line when it comes to combining the results.
...
`.semaphore/upload_test_artifacts.sh`:
...
`.semaphore/calc_code_coverage.sh`:
...
Finally here's what your Semaphore workflow artifacts will look like. This approach can also be easily ported over to other CI providers by simply changing the `artifact push` and `artifact pull` commands to S3 or another CI specific artifact upload command.
